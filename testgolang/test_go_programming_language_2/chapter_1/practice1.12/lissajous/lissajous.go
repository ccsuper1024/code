...
func Lissajous(out io.Writer, req *http.Request) {
	//number of complete x oscillator revolutions	正当其的完整震荡周期数
	// var cycles int

	cyclesStr := req.URL.Query().Get("cycles")
	cycles, err := strconv.Atoi(cyclesStr)
	if err != nil {
		fmt.Printf("cycles = %d\n", cycles)
		fmt.Fprintf(out, "Url:%s 中的字段错误: %v", req.URL, err)
		os.Exit(1)
	}

	fmt.Fprintf(out, "Get Cycles")
