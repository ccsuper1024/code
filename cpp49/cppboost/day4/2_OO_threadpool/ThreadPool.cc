...
,_isExit(false)
{
    _threadpool.reserve(_threadNum);
...
void ThreadPool::start() {
    ...
    for(size_t idx = 0; idx < _threadNum; ++idx) {
        unique_ptr<Thread> th(new WorkThread(*this));   //WorkThread(*this)会调用构造函数
...
void ThreadPool::doTask() {
    //线程池中的工作线程调用run函数后，会执行此函数。那么在此函数中就需要判定
    //线程池中是否有任务。即通过getTask();再取执行具体的使用者给代码，即执行MyTask
    //中的虚函数process(),这会引发多态。
    ...
    while(!_isExit) {
        ...
        if(pTask) {
            //执行具体的任务
            pTask->process();   //基类指针指向派生类，再一次多态而WorkThread中的Thread抽象类，也会在threadFunc中也调用多态
            //函数调用：Thread::threadFunc()::t->run();-->
            //WorkThread::run()::_pool.doTask();-->ThreadPool::doTask()::-->pTask->process();
        }
    }
}