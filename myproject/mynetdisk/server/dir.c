...
int curDirInit(MYSQL* mysql,user_t* usInfo){
    ...
    char sql[128]={0};
    sprintf(sql,"%s%s%s","select * from filetable where tomb=0 and pre_id=-1 and user='",userName,"';");
    int qret=mysql_query(mysql,sql);
    ...
    if(mysql_num_rows(res)==0){
        ...
        //根据插入结果获取数据
        bzero(sql,sizeof(sql));
        sprintf(sql,"%s%s%s","select * from filetable where tomb=0 and user='",userName,"';");
        mysql_query(mysql,sql);
        printf("sql=%s\n",sql);
        res = mysql_store_result(mysql);
    }
...
int cdDir(MYSQL* mysql,user_t* userInfo,char* str){
    ...
    if (str[0]=='/'){//绝对路径的话
        ...
        //先根据根目录找到第一个
        bzero(sql,sizeof(sql));
        sprintf(sql,"%s%d%s%s%s","select * from filetable where tomb=0 and type='d' and pre_id=",userInfo->rootDirId," and name='",stack->elements[0],"';");
        ...
        for(int i=1;i<stack->top;i++){
            bzero(sql,sizeof(sql));
            sprintf(sql,"%s%d%s%s%s","select * from filetable where tomb=0 and type='d' and pre_id=",userInfo->curDirId," and name='",stack->elements[i],"';");
            int ret=changeCurID(mysql,sql,userInfo);
            if(ret==-1){
            userInfo->curDirId=tmp;
                free(stack);
        ...
        for(int i=0;i<stack->top;i++){
            ...
            if(strcmp(stack->elements[i],"..")==0){//退回一级目录,根据当前id找pre_id
                if(userInfo->curDirId==userInfo->rootDirId){
                    continue;
                }
                sprintf(sql,"%s%d%s","select pre_id from filetable where tomb=0 and type='d' and id=",userInfo->curDirId,";");
                int ret=changeCurID(mysql,sql,userInfo);
                if(ret==-1){
                    userInfo->curDirId=tmp;
                    free(stack);
...
int mkDir(MYSQL*mysql,user_t* userInfo,char* str){
    char sql[256]={0};
    sprintf(sql,"%s%d%s","select name from filetable where tomb=0 and pre_id=",userInfo->curDirId,";");
    printf("sql=%s\n",sql);
    mysql_query(mysql,sql);
    MYSQL_RES* res=mysql_store_result(mysql);
    MYSQL_ROW row;
...
int list(user_t userinfo,MYSQL* sqlhandler ,const char* userbuff, int netfd)
{
    ...
    if(userbuff[0]=='\0')
    {
        sprintf(sql,"%s%s%s%d;","select name from filetable where tomb=0 and user='"
    ...
    else
    {
        ...
        sprintf(sql,"%s%s%s'%s';","select id from filetable where tomb=0 and user='",
        ...
        char strid[16]={0};
        memcpy(strid,row[0],strlen(row[0]));
        sprintf(sql,"select name from filetable where tomb=0 and pre_id=%s;",strid);
    }
    ...
    MYSQL_ROW row;
    char sendstr[512]={0};
    for(int i=0;i<rowNum&&row!=NULL;i++){
        row=mysql_fetch_row(res);
        strcat(sendstr,row[0]);
        strcat(sendstr,"  ");
        /* printf("i=%d row[0]=%s",i,row[0]); */
        /* send(netfd,row[0],sizeof(row[0]),0); */
        /* send(netfd,"\t",2,0); */
    }
    send(netfd,sendstr,strlen(sendstr),0);
...
int removeThing(user_t userinfo ,MYSQL* sqlhandler,const char* userbuff,int netfd)
{
    ...
    char sql[1024]={0};
    memset(sql,0,sizeof(sql));
    sprintf(sql,"select * from filetable where tomb=0 and user='%s' and path='%s';",
            username,abspath);
    printf("rm:sql=%s",sql);
    mysql_query(sqlhandler,sql);
    MYSQL_RES* res=mysql_store_result(sqlhandler);
    ...
    bzero(sql,sizeof(sql));
    sprintf(sql,"UPDATE filetable SET tomb = -1  where user ='%s' and path='%s';"
            ,username,abspath);
